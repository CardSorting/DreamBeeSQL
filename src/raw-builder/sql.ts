...
export interface Sql {
   ...
   *
   * ⚠️ DANGER: This method is UNSAFE and should ONLY be used with trusted,
   * hardcoded values. It does NOT escape or validate input. Using this with
   ...
   *
   * 🚨 EXTREME DANGER: This method is EXTREMELY UNSAFE and should NEVER be
   ...
   *
   * // ✅ Or with SQL fragments from trusted sources:
   * const sortOrder = 'DESC'  // Validated against ['ASC', 'DESC']
   * sql`select * from person order by name ${sql.raw(sortOrder)}`
   * ```
   *
   ...
   * - Can I use sql.ref(), sql.table(), or sql.id() instead?
   * - Is this input from a 100% trusted source?
   */
